...
i_1m = beta_1_m_geo - beta_1_m_od;
    ...
    alpha_2_iter = atand(V_2T_m/V_2A_m(k));
    l_Eu_m = U_m * (V_2T_m - V_1T_m);
    T_T2_m = T_T1_m + l_Eu_m / cp;
    ...
    while abs( (attack_c_m(end) - attack_c_m(end-1))/attack_c_m(end-1) ) > tol
        ...
        attack_c_m(end+1) = attack_m_design - 9 + ( 1 - (30/beta_c_m)^0.48 )*teta_mid/4.176;
   ...
   attack_s_m = [attack_m_design*2 attack_m_design];
    while abs( (attack_s_m(end) - attack_s_m(end-1))/attack_s_m(end-1) ) > tol
        ...
        attack_s_m(end+1) = attack_m_design + 10.3 + ( 2.92 - (beta_s_m/15.6) )*teta_mid/8.2;
    ...
    for i = 2:length(beta_1_geo_high)
        ...
        V_2A_iter = [V_2A_high(i-1)*2 V_2A_high(i-1)];
    while abs( (V_2A_iter(end) - V_2A_iter(end-1))/V_2A_iter(end-1) ) > tol
        V_2A_iter(end-1) = V_2A_iter(end);
        
    W_2A_iter = V_2A_iter(end);
    ddelta_di_iter = ( 1+(sigma_R_high(i)+0.25*sigma_R_high(i)^4)*(abs(beta_1_high(i))/53)^2.5 )/exp(3.1*sigma_R_high(i));
    delta_rot_od_iter = dev_opt_rot_high(i) + ddelta_di_iter * (i_1_high(i) - i_opt_rot_high(i) ) + 10*(1-W_2A_iter/W_1A_high(i));
    ...
    l_Eu_iter = omega * rhigh(i) * (V_2T_iter - V_1T_high(i));
    T_T2_iter = T_T1_high(i) + l_Eu_iter / cp;
    T_2_iter = T_T2_iter - V_2_iter^2 / 2 / cp;
    ...
    while abs( (attack_c_iter(end) - attack_c_iter(end-1))/attack_c_iter(end-1) ) > tol
        ...
        attack_c_iter(end+1) = attack_design_high(i) - 9 + ( 1 - (30/beta_c_iter)^0.48 )*teta_high(i)/4.176;
   ...
   attack_s_iter = [attack_design_high(i)*2 attack_design_high(i)];
    while abs( (attack_s_iter(end) - attack_s_iter(end-1))/attack_s_iter(end-1) ) > tol
        ...
        attack_s_iter(end+1) = attack_design_high(i) + 10.3 + ( 2.92 - (beta_s_iter/15.6) )*teta_high(i)/8.2;
    ...
    p_2_iter  = p_2_high(i-1) / ( 1 -  V_2T_iter^2 / rhigh(i) / R_star / T_2_iter * Drhigh );
    rho_2_iter = p_2_iter / R_star / T_2_iter;
    p_T2_iter = p_2_iter * ( (1+(gamma-1)/2*(rho_2_iter*V_2_iter^2/(gamma*p_2_iter)))^(gamma/(gamma-1)) );
    V_2_iter = sqrt( 2*gamma*R_star*T_2_iter / (gamma-1) * ( (p_T2_iter/p_2_iter)^((gamma-1)/gamma) - 1 ) );
            
            T_T2_iter = T_2_iter + V_2_iter^2/2/cp;
            l_Eu_iter = cp*( T_T2_iter - T_T1_high(i) );
            V_2T_iter = V_1T_high(i) + l_Eu/omega/rhigh(i);
            V_2A_iter(end+1) = sqrt( V_2_iter^2 - V_2T_iter^2 );
            
            % V_2A_iter(end+1) = V_2_iter*cosd(alpha_2_iter);
        
    end

    V_2A_high(i)    =  V_2A_iter(end);
    ...
    beta_2  =  [beta_2_low(end:-1:1) beta_1_high(2:end) ];

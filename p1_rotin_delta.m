...
V_1A_m = linspace(V_1A(end)*0.8,V_1A(end)*1.2,1000);
       
    for k = 1:length(V_1A_m)
    
    W_1A_m = V_1A_m(k);
    W_1T_m = W_1A_m * tand(beta_1_m_od);
    V_1T_m = W_1T_m + omega * D_m/2;
    alpha_1_m = atand(V_1T_m/V_1A_m(k));
    V_1_m = sqrt(V_1A_m(k)^2+V_1T_m^2);
...
%         alpha_1_iter = alpha_1_geo_od_low(i) - delta_od_IGV_low(i);
    ...
    sol = find( (mnew > m*0.999) & (mnew < m*1.001) );
    k = sol(end);
